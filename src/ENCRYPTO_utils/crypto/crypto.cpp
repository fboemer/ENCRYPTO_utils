...
void crypto::gen_rnd_uniform(uint32_t* res, uint32_t mod)
{
    ...
    uint32_t nrndbytes =
        PadToMultiple(bits_in_bytes(secparam.symbits) + ceil_log2(mod), sizeof(uint32_t));
...
void crypto::init_aes_key(AES_KEY_CTX* aes_key,
                          uint32_t symbits,
                          uint8_t* seed,
                          bc_mode mode,
                          const uint8_t* iv,
...
void crypto::seed_aes_key(
    AES_KEY_CTX* aeskey, uint8_t* seed, bc_mode mode, const uint8_t* iv, bool encrypt)
{
...
void crypto::seed_aes_key(AES_KEY_CTX* aeskey,
                          uint32_t symbits,
                          uint8_t* seed,
                          bc_mode mode,
                          const uint8_t* iv,
...
#endif
    int (*initfct)(
        EVP_CIPHER_CTX*, const EVP_CIPHER*, ENGINE*, const unsigned char*, const unsigned char*);
...
void crypto::hash_ctr(
    uint8_t* resbuf, uint32_t noutbytes, uint8_t* inbuf, uint32_t ninbytes, uint64_t ctr)
{
...
void crypto::hash_buf(
    uint8_t* resbuf, uint32_t noutbytes, uint8_t* inbuf, uint32_t ninbytes, uint8_t* buf)
{
    hash_routine(resbuf, noutbytes, inbuf, ninbytes, buf);
}

void crypto::hash_non_threadsafe(uint8_t* resbuf,
                                 uint32_t noutbytes,
                                 uint8_t* inbuf,
                                 uint32_t ninbytes)
{
...
void crypto::fixed_key_aes_hash(
    AES_KEY_CTX* aes_key, uint8_t* resbuf, uint32_t noutbytes, uint8_t* inbuf, uint32_t ninbytes)
{
    int32_t dummy;
...
void crypto::init_prf_state(prf_state_ctx* prf_state, uint8_t* seed)
{
    ...
    prf_state->ctr =
        (uint64_t*)calloc(ceil_divide(secparam.symbits, 8 * sizeof(uint64_t)), sizeof(uint64_t));
}
...
void sha1_hash(
    uint8_t* resbuf, uint32_t noutbytes, uint8_t* inbuf, uint32_t ninbytes, uint8_t* hash_buf)
{
    SHA_CTX sha;
    SHA1_Init(&sha);
